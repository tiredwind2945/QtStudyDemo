      ...
      autoAcceptLongestCommonCompletionPrefix_(true)
{
    ...
    connect(this, SIGNAL(escapePressed()), this, SLOT(onEscapePressed()));
    connect(this, SIGNAL(upPressed()), this, SLOT(onUpPressed()));
    connect(this, SIGNAL(downPressed()), this, SLOT(onDownPressed()));
    connect(this, SIGNAL(tabPressed()), this, SLOT(onTabPressed()));
    connect(this, SIGNAL(shiftTabPressed()), this, SLOT(onShiftTabPressed()));
...
void QCommandEdit::setHistoryIndex(int index)
{
    ...
    QTimer::singleShot(0, this, SLOT(moveCursorToEnd()));
    setToolTipAtCursor("");
}
    ...
    if(autoAcceptLongestCommonCompletionPrefix_)
    {
        ...
        if(!lcp.isEmpty() && completionState_.requested_)
        {
            ...
            for(int i = 0; i < completion.size(); i++)
                completionTrimmed.append(completion[i].mid(lcp.length()));
